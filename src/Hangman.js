...
module.exports = class Hangman extends events {
  constructor(options = {}) {
    ...
    if (!options.embed.title) options.embed.title = 'Homem da Forca';
    ...
    if (!options.winMessage) options.winMessage = 'Você ganhou! A palavra era **{palavra}**.';
    ...
    if (options.playerOnlyMessage !== false) {
      if (!options.playerOnlyMessage) options.playerOnlyMessage = 'Apenas o {player} pode usar esses botões.';
  ...
  async startGame() {
    ...
    .addFields({ name: `Palavra (${this.word.length})`, value: this.getWordEmojis() });
  ...
  handleButtons(msg) {
    ...
    collector.on('collect', async btn => {
      ...
      .setDescription(this.getBoardContent())
      .setAuthor({ name: this.message.author.tag, iconURL: this.message.author.displayAvatarURL({ dynamic: true }) })
      .addFields({ name: 'Letras adivinhadas:', value: '`'+ this.guessed.join(', ') +'`' })
      .addFields({ name: `Letra: (${this.word.length})`, value: this.getWordEmojis() });
  ...
  gameOver(msg, result) {
    ...
    this.emit('gameOver', { result: (result ? 'Venceu' : 'Perdeu'), ...HangmanGame });
    ...
    if (this.guessed.length) embed.addFields({ name: 'Letrad adivinhadas', value: '`'+ this.guessed.join(', ') +'`' });
    embed.addFields({ name: 'Fim do jogo', value: GameOverMessage.replace('{word}', this.word) });
  ...
  getComponents(page) {
    ...
    const stop = new ButtonBuilder().setStyle('DANGER').setLabel('Parar').setCustomId('hangman_stop');
